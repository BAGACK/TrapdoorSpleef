...
import net.minecraft.server.v1_10_R1.BlockTrapdoor;
import net.minecraft.server.v1_10_R1.BlockTrapdoor.EnumTrapdoorHalf;
...
public class NMSHandler110 implements NMSAbstraction {
	...
	public void queueChunkForUpdate(Player player, int cx, int cz) {
		...
		((CraftPlayer) player).getHandle().playerConnection.sendPacket(new PacketPlayOutMapChunk(((CraftChunk)chunk).getHandle(), '\uffff'));
	}
