...
public class CraftMassBlockUpdate implements MassBlockUpdate, Runnable {
	...
	public CraftMassBlockUpdate(Plugin plugin, org.bukkit.World world) {
		this.plugin = plugin;
		this.world = world;
		this.nms = VersionManager.getNMSHandler(MinigamesAPI.SERVER_VERSION);
		if (nms == null) {
			throw new IllegalStateException("NMS abstraction API is not available");
		}
	}

	public boolean setWoodenTrapDoor(int x, int y, int z, int data) {
		...
		boolean res = nms.setWoodenTrapDoorFast(world, x, y, z, (byte) data);

		if (relightingStrategy != RelightingStrategy.NEVER) {
			if (nms.getLightBlocking(oldBlockId) != nms.getWoodenTrapDoorLightBlocking() || nms.getLightEmission(oldBlockId) != nms.getWoodenTrapDoorLightEmission()) {
